...
loginForm.addEventListener("submit", async (event) => {
    ...
    if (response) {
        // crear objeto de sesión con la forma esperada por perfil.html
        const usuario = {
            idUsuario: Number(
                response.idUsuario ??
                response.id ??
            ...
            correo: response.correo ?? response.usuario?.correo ?? ""
        };

        // guardar en sessionStorage para que perfil.html pueda leerlo
        sessionStorage.setItem("usuario", JSON.stringify(usuario));
        // mantener la entrada previa en localStorage para la lógica del dashboard
